...
router.post(
    ...
    asyncHandler(async (req, res) => {
        ...
        if (!device.activation_code || device.activation_code.toUpperCase() !== cleanCode) {
...
router.post(
    ...
    asyncHandler(async (req, res) => {
        ...
        const result = await pool.query(
            "SELECT id, device_name, phone, gate, fingerprint_hash, device_token, status, last_active_at FROM guard_devices WHERE id = $1",
            [device_id]
        );
        ...
        if (device.device_token && device_token !== device.device_token) {
            return res.status(200).json(
