...
// Note: In hostel-backend, room table doesn't have an enum constraint by default, but it's good to keep it consistent.
const hostelsData = [
    {
    ...
    },
    {
        ...
        rooms: [
            ...
            { block: 'Admin', prefix: 'D-G', start: 1, end: 2, capacity: 2, roomType: 'Student' }
        ]
    },
    {
    ...
    },
    {
        ...
        rooms: [
            ...
            { block: 'Admin', prefix: 'P-G', start: 1, end: 2, capacity: 2, roomType: 'Student' },
            { block: 'Admin', prefix: 'P-VIS', start: 1, end: 1, capacity: 4, roomType: 'Student' }
        ]
    },
    {
...
async function seedHostels() {
    ...
    try {
        ...
        for (const hostel of hostelsData) {
            ...
            // In hostel-backend, we might have conflicts on hostel name if run multiple times.
            let hostelId;
            const existingHostel = await client.query('SELECT id FROM hostel WHERE name = $1', [hostel.name]);
            
            if (existingHostel.rows.length > 0) {
                hostelId = existingHostel.rows[0].id;
                console.log(`  Hostel already exists (ID: ${hostelId}), deleting existing rooms to re-seed...`);
                await client.query('DELETE FROM room WHERE hostel_id = $1', [hostelId]);
            } else {
